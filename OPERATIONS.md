...
## Round 2 operations notes

### Prisma generation in CI

Do not commit generated Prisma Client. Preferred CI options:

1. Run CI with network access to `binaries.prisma.sh`.
2. Cache npm plus Prisma engines across runs.
3. Use a prebuilt CI Docker image that already contains npm dependencies and Prisma engines.
4. Run `npm run db:generate` before lint/typecheck/build.

The sandbox used for this patch cannot resolve Prisma engine downloads, so `db:generate`, `lint`, `typecheck` and `build` are blocked there.

### Outbox dispatcher scale

Multiple dispatchers can run. Each dispatcher must atomically claim an event by moving it to `PROCESSING` with `claimedBy` and `processingStartedAt`. Stale processing events are eligible for recovery after timeout.

### Action heartbeat

The action execution service renews `heartbeatAt` and `lockExpiresAt` periodically while executing. A crashed worker leaves the lock to expire naturally and another worker can reclaim it.
