...
JWT access tokens support no-downtime secret rotation through `JWT_SECRET` for signing and `JWT_PREVIOUS_SECRETS` for verification. Cookie-auth mutating requests require both Origin/Referer validation and double-submit CSRF token (`csrfToken` cookie + `x-csrf-token` header). `@fastify/helmet` is enabled. Password policy helpers reject weak or obvious passwords before hashing.
